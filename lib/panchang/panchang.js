...
var z = floor(abs(fix360(Lmoon + ayanamsa))/30);
    ...
    // Added: n_tithi is already correctly computed above (0-14 = Shukla
    // Paksha, 15-29 = Krishna Paksha) — exposing it since several tithi
    // names repeat across both halves of the array, so callers can't
    // otherwise tell the two apart from tith[] alone.
    this.Tithi.paksha = n_tithi < 15 ? "Shukla" : "Krishna";
    if (cb) { cb();}
}
}

// Added for use as a Node module — purely additive, nothing above this
// line was touched, so the calculation logic is untouched from the
// original working browser script.
